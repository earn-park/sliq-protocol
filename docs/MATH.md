...
This volatility signal is currently observable off-chain by analyzing skew equilibrium data. On-chain IV derivation from the K-multiplier state is an active development effort (see [ROADMAP.md](./ROADMAP.md)).

### Pricing Model Evolution: From Anchor Fees to GBM-Based Implied Volatility

The v1 protocol (current codebase) prices positions using **anchor fee accrual** — the actual trading fees generated by the anchor Uniswap V3 NFT position flow into the checkpoint system and are distributed to Long/Short positions via the K-multiplier.

The v2 pricing model, currently under development, decouples fee pricing from the anchor position entirely and replaces it with a **GBM-based implied volatility model**:

1. **Fee source**: Instead of deriving fees from the anchor's Uniswap V3 fee growth accumulators, the protocol computes an internal **implied volatility** from the current K-multiplier equilibrium state and market conditions.

2. **Pricing formula**: The IL payoff formula ($\text{IL\%}(r) = 1.0001^r - \sqrt{1.0001^r}$) remains unchanged — it is the core non-linear payoff that makes sLiq a volatility instrument. What changes is the **fee component**: instead of `totalFeeCum` sourced from the anchor, fees are calculated as a function of implied volatility, position range, and time elapsed. The mathematical basis is the same log-normal price dynamics described above (the discrete GBM encoded in the tick grid), but applied to price the cost of IL exposure directly.

3. **Implication for LPs**: In v2, LP deposits back the protocol's obligations rather than a specific Uniswap V3 position. LP yield derives from the spread between implied and realized volatility — analogous to an options market maker's edge.

4. **Implication for traders**: The user-facing interface remains identical (Long/Short, range, collateral). The change is internal: fees are displayed as projected cost/income per unit of time based on current implied volatility, rather than as a raw K-multiplied share of anchor fees.

The v1 anchor-fee model is fully functional and was used in the live beta (2,400+ positions, 175 ETH collateral). The v2 model preserves the same IL formula, K-multiplier skew mechanism, and position lifecycle — only the fee pricing source changes.
