...
- **Set fee parameters**: `setFees(vaultE2, protocolE2, liquidatorE18)` with no upper bounds enforced on-chain. A vault owner could set fees to 100%, making all positions unprofitable.
- **Receive protocol fees**: Protocol fee share of closed positions is sent to the vault owner address.

**Recommended mitigations**:
- Add on-chain fee caps (e.g., combined fees <= 10%)
...
- **Price feed**: Used as the primary price source when the sequencer is healthy. Standard Chainlink staleness checks are applied (round completeness, positive answer, non-zero update time).
- **Fallback**: If Chainlink is unavailable, `pool.slot0()` is used. This is susceptible to sandwich attacks and flash loan manipulation, but is the standard fallback for Arbitrum protocols.
...
- **NFT position**: The anchor position must remain valid and owned by the vault. If the NFT is transferred out or the position is burned externally, the vault would break.

## Known Limitations

### 1. No Fee Caps

`setFees()` accepts arbitrary values with no on-chain maximum. The vault owner could set `feeVaultPercentE2 + feeProtocolPercentE2 = 10000` (100%), leaving nothing for positions. This is currently a trust assumption on the vault owner.
...
### 3. Share Accounting Edge Cases

The deposit function has a floor guarantee (`shares >= amount`) to prevent dilution. However:
...
There is no on-chain pause mechanism. The vault owner can influence the system through fee parameters:

- **Soft pause via fees**: Set `feeVaultPercentE2 = 10000` to make new positions economically unviable. This does not prevent opens/closes but removes the incentive.
- **Implementation upgrade**: The VaultManager owner can deploy a new implementation with a `pause()` function and upgrade all vaults to it.

### Recommended Additions

1. **Pausable pattern**: Add OpenZeppelin's `PausableUpgradeable` to Vault. Allow the owner to pause `openLong`, `openShort`, `deposit`, and `withdraw` independently.
2. **Guardian role**: A separate address (faster than the owner multisig) that can only pause, not unpause or change parameters.
3. **Timelock on unpause**: Require a delay before unpausing to give users time to exit.
4. **Circuit breaker**: Automatically pause if single-block PnL exceeds a threshold.
